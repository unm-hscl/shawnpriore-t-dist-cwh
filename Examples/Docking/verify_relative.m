function [P_target, P_2v, P_1v] = verify_relative(mean_traj, Wd_concat, psi, nu, time_horizon, target_sets, r, samples)
    ...
    collision_1v = zeros(vehicles, time_horizon, samples);
    for i = 1:vehicles
        for t = 1:time_horizon
            time_index = 6*(t-1) + [1:3];
            for k = 1:samples
                    collision_1v(i, t, k) = ( norm( disturbed_mean(time_index, i, k)) >= r); 
            end
        end
    end
    P_1v = sum(sum(sum(collision_1v, 1) == vehicles, 2) == time_horizon) / samples; 
end